...
function MenuLink({ icon, label, onClick, danger = false, highlight = false }) {
  return (
    <motion.button
      ...
      className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-medium transition-colors group ${danger ? "text-red-600" : highlight ? "text-blue-600 bg-blue-50/50" : "text-gray-600"
        }`}
    >
  ...
  return (
    <>
      <motion.nav
        ...
        initial="hidden"
        animate="visible"
        className={`fixed top-0 inset-x-0 z-[100] transition-all duration-300 ${scrolled || mobileMenuOpen
