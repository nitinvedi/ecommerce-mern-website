...
export default function ManageRepairs() {
   ...
   // Real-time updates
   useEffect(() => {
      const handleNewRepair = (data) => {
         toast.info(`New Repair Request: ${data.trackingId}`);
         fetchRepairs();
      };

      const unsubscribe = on('new_repair', handleNewRepair);
      return () => {
                        ...
                        <div className="space-y-6">
                           <div>
                              ...
                              <div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                                 <p className="font-medium capitalize">{activeRepair.device?.brand} {activeRepair.device?.model}</p>
                                 <p className="text-sm text-gray-600 mt-1">Serial: {activeRepair.device?.serialNumber || "N/A"}</p>
                                 <p className="text-sm text-gray-600 mt-2 italic capitalize">"{activeRepair.issueDescription}"</p>
                              </div>
                           </div>
                           ...
                           <div>
                              ...
                              <div className="flex items-center gap-3">
                                 ...
                                 <div>
                                    <p className="font-medium text-sm capitalize">{activeRepair.user?.name}</p>
